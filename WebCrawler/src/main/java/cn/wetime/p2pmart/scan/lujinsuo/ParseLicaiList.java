...
public class ParseLicaiList extends  AbsLujinsuoList  {

	private static final String URL = "https://list.lufax.com/list/licai";
		...
		for (int i = 1; i <= pages; i++) {
			...
			for (int j = 0; j < listsLen; j++) {
                ...
                if(j==0)
                	...
                	if(progressLists!=null && progressLists.size()!=0 && progressLists.size()>=j){
	                	String progressTxt = progressLists.elementAt(j-1).toPlainTextString();
				...
				//设置平台名称
				product.setPlatformName("陆金所-理财");
