...
public class HtmlPicker implements DataPicker {
	...
	public String get() {
		...
		if(limitRule.matches("\\d,html\\[.+\\]")){
			...
			int rdIdx = limitRule.indexOf("[");	int thIdx = limitRule.lastIndexOf("]");
		...
		if(htmlRt!=null&&!htmlRt.equals("")){
			html = htmlRt;pickAgent.setObeyRule(true);
			if(regExp!=null && !regExp.equals("")) html = MatchUtils.getOnlyMatchs(html, regExp, 1);
		}
		return html;
	}
